...
# Create subgroups for jumping and walking data under each person's group
jack_jumping_group = jack_group.create_group('jumping')
jack_walking_group = jack_group.create_group('walking')
kieran_jumping_group = kieran_group.create_group('jumping')
kieran_walking_group = kieran_group.create_group('walking')
amir_jumping_group = amir_group.create_group('jumping')
amir_walking_group = amir_group.create_group('walking')

# Store jumping and walking segments under each person's group
jack_jumping_group.create_dataset('data', data=jack_jumping_segments)
jack_walking_group.create_dataset('data', data=jack_walking_segments)
kieran_jumping_group.create_dataset('data', data=kieran_jumping_segments)
kieran_walking_group.create_dataset('data', data=kieran_walking_segments)
amir_jumping_group.create_dataset('data', data=amir_jumping_segments)
amir_walking_group.create_dataset('data', data=amir_walking_segments)
...
training_jumping_group = training_group.create_group('jumping')
training_jumping_group.create_dataset('data', data=shuffled_jumping_segments[0])
training_walking_group = training_group.create_group('walking')
training_walking_group.create_dataset('data', data=shuffled_walking_segments[0])

# Store shuffled jumping and walking segments in Testing group
testing_group = dataset_group.create_group('Test')
testing_jumping_group = testing_group.create_group('jumping')
testing_jumping_group.create_dataset('data', data=shuffled_jumping_segments[1])
testing_walking_group = testing_group.create_group('walking')
testing_walking_group.create_dataset('data', data=shuffled_walking_segments[1])
